...
from constants import LABELS, Frases, SCREENSIZE, WIDTH, HEIGHT, FireBall_constants, MageFireBall_constants, \
    Spell_constants, Tree_constants, Enemy_constants, Mage_constants
...
class MageFireBall(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, phi, *groups):
        ...
        self.rect.x = x
        self.rect.y = y
        self.phi = phi # угол, под которым полетит фаерболл
    ...
    def fire(self):
        ...
        if x == self.rect.x:
            if y - self.rect.y > 0:
                ...
                phi = - math.pi / 2
        MageFireBall(self.rect.x + 35, self.rect.y + 10, phi, all_sprites, mage_fireballs)
...
class Timer:
    ...
    def update(self):
        ...
        if self.time < time.time():
            ...
            if self.color == Spell_constants.BLUE:
                for i in enemy_group:
                    i.slowing = Spell_constants.slow_motion_alpha #замедляем врагов
                for i in mages_group:
                    i.slowing = Spell_constants.slow_motion_alpha #замедляем магов
            elif self.color == Spell_constants.GREEN:
